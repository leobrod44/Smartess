...
import BackArrowButton from "@/app/BackArrowBtn";
...
export default function IndividualUnitSurveillancePage() {
  ...
  const decodedAddress = "1000 De La Gauchetiere";
  const unit_id = "101";
  ...
  const recordedBuffersRef = useRef<{timestamp: number, buffer: ArrayBuffer}[]>([]); // Store timestamped buffers
  const lastSeekTimeRef = useRef<number | null>(null);
  const seekingBackwardsRef = useRef<boolean>(false);
  // ------------------------------------------------------------------------------

  // Move showStatus outside useEffect so it can be used by other functions
      ...
      <div className="flex-1 border border-black rounded-lg p-6 mx-4 lg:mx-8 max-h-screen flex flex-col">
        ...
        <div className="my-5 flex flex-col md:flex-row gap-4 justify-between">
          ...
          <div className="w-full md:w-1/4 flex flex-col gap-3 h-full">
            ...
            <div className="flex flex-col gap-3 flex-grow text-black">
              {/* Speed Section */}
