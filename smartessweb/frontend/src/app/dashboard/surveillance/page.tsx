...
const SurveillancePage = () => {
  ...
  const [widgetFilter, setWidgetFilter] = useState<"all" | "live" | "disconnected">("all");
...
const fetchData = async () => {
  ...
  const filteredByCameraStatus = filteredUnitsByProject.filter((unit) => {
    ...
    if (widgetFilter === "disconnected" && (unit.cameraStatus === "disconnected")) return true;
    return false;
  });
  ...
  const liveCamerasCount = allUnits.filter((unit) => unit.cameraStatus === "live").length;
  const inactiveCamerasCount = allUnits.filter((unit) => unit.cameraStatus === "disconnected").length;
                ...
                <p className="text-sm flex items-center">
                  ...
                  <span
                    className={`w-3 h-3 rounded-full ml-2 ${
                      unit.cameraStatus === "live" ? "bg-green-500" : "bg-red-500"
                    }`}
                  />
                </p>
              </div>
