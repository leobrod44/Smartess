 ...
 *
 * This function checks if the user exists, creates a reset token in the temp_acc_access table,
 * and sends a password reset email using Resend.
 *
 * @param {Object} req - The HTTP request object containing:
 ...
 *
 * @returns {Response} - A JSON response:
 *   - Success: `{ message: "Password reset email sent successfully." }` (HTTP 200)
 *   - User not found: `{ error: "User not found." }` (HTTP 404)
 *   - Other errors: `{ error: "..." }` (HTTP 500)
 *
 * @throws {Error} If an error occurs during the process, it logs the error and returns a 500 response.
 */
exports.sendPasswordReset = async (req, res) => {
  try {
    ...
    const { error: deleteError } = await supabase
      .from('temp_acc_access')
      .delete()
      .match({ task: 'forgot-password', email: email });
    ...
    const subject = `Smartess Password Reset`;
    const webURL = process.env.WEBSITE_URL;
          ...
          <div class="email-container">
            ...
            <div class="email-body">
              ...
              <div class="instructions">
                ...
                <a href="${webURL}/reset-password?${token}" class="reset-button">Reset Your Password</a>
